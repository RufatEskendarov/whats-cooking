...
const addNewRecipe = async (event) => {
  ...
  const cookingTime = document
    .querySelector("#recipe-cooking-time")
    .value.trim();

  const ingredientFields = document.querySelectorAll(
    "#ingredient-column input"
  );

  const ingredients = Array.from(ingredientFields)
